...
// El servidor (Vercel) corre en horario UTC, sin importar dónde esté el
// negocio. Panamá es UTC-5 todo el año (no tiene horario de verano), así
// que para registrar "la fecha de hoy" correctamente en la base de datos
// (firma del contrato, devolución real, etc.) hay que pedirle
// explícitamente la fecha en el huso horario de Panamá, no la del
// servidor — si no, después de las 7:00pm hora de Panamá (medianoche UTC)
// se registraría el día siguiente por error.
export function fechaHoyPanama() {
...
// tipo de vehículo, usando la configuración vigente y multiplicando por
// la cantidad de días de la renta. Devuelve tanto el total como el
// detalle (para guardar el precio "congelado" en la renta).
export function calcularExtras(idsSeleccionados, config, tipoVehiculo, dias = 1) {
  const cfg = config || EXTRAS_COBERTURAS_POR_DEFECTO;
  const diasCobrados = Math.max(1, dias || 1);
  ...
  const detalle = (cfg.extras || [])
    ...
    .map((e) => ({
      ...
      precioPorDia: e[campoPrecio],
      dias: diasCobrados,
      precio: Math.round(e[campoPrecio] * diasCobrados * 100) / 100,
      devuelto: null,
    }));
...
export function calcularTarifa({ tipoMoto, fechaEntrega, horaEntrega, fechaPrevista, tarifas }) {
  ...
  if (dias <= 0) {
    if (hora >= 14) return { total: t.especial, regla: "especial", dias: 1 };
    return { total: t.dia, regla: "dia", dias: 1 };
  }
  if (hora >= 17) {
